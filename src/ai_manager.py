...
class AiManager:
    ...
    def ask_ollama_langchain_rag(self, prompt, translate=True):
        ...
        if self.rag.retriever:
            ...
            self.rag_chain = RetrievalQA.from_chain_type(
                llm=self.llm, chain_type="stuff", retriever=self.rag.retriever, return_source_documents=True
            )
        else:
...
if __name__ == "__main__":
    ...
    response3 = ai_manager.ask_ollama_langchain_rag(
        "Что такое контекст-менеджер в Python? Приведи пример использования.", translate=True
    )
